...
PELICULAS_INFO = {
    ...
    'Regreso al Futuro': {
        ...
        'sinopsis': 'Marty McFly viaja accidentalmente a 1955 en un DeLorean inventado por Doc Brown y debe asegurar que sus padres se enamoren para no desaparecer.',
    ...
    'Karate Kid': {
        ...
        'sinopsis': 'Un adolescente aprende artes marciales de la mano de un sabio maestro japonés para defenderse de unos acosadores en el instituto.',
    ...
    'Star Wars': {
        ...
        'sinopsis': 'Luke Skywalker se une a un caballero Jedi y a un piloto fanfarrón para salvar a la galaxia de la estación de combate del Imperio.',
    ...
    'Terminator': {
        ...
        'sinopsis': 'Un cyborg asesino viaja desde el futuro para matar a Sarah Connor, cuyo hijo no nacido será la clave de la resistencia humana.',
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            db.commit()
            flash('¡Cuenta creada! Ya puedes entrar.')
...
def nuevo_post():
    ...
    if request.method == 'POST':
        ...
        db = get_db_connection()
        db.execute('INSERT INTO posts (titulo, contenido, autor_id) VALUES (?, ?, ?)', (titulo, contenido, session['user_id']))
        db.commit()
        db.close()
        return redirect(url_for('dashboard'))
...
def borrar_post(post_id):
    if 'user_id' not in session: return redirect(url_for('login'))
    db = get_db_connection()
    db.execute('DELETE FROM posts WHERE id = ? AND autor_id = ?', (post_id, session['user_id']))
    db.commit()
    db.close()
    return redirect(url_for('dashboard'))
